...
`mvn install -Pdev -Dusername=<apigee-username> -Dpassword=<apigee-password> -Dapigee.config.options=update`

To run the plugin and use the multi-file config format jump to samples project `cd /nseindia/config` and run 

`mvn install -Pdev -Dusername=<apigee-username> -Dpassword=<apigee-password> -Dapigee.config.options=update -Dapigee.config.dir=resources/edge`
...
Each config entity is the payload of the corresponding management API. The project may not show all the attributes of config entities, but any valid input of a management API would work.
